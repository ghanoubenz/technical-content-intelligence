// V5 — Step 1: V2 content, V4 layout, NO HR sheet, NO HR column. Prompts unchanged from V2.
...
async function build() {
  ...
  wb.creator = 'ACME Integrity Strategy V5';
  wb.created = new Date();
  ...
  const pillarColors = {
    'TECH': fillSolid('FFE3F2FD'),
    'PEOPLE': fillSolid('FFF3E5F5'),
    'INSIGHT': fillSolid('FFE8F5E9'),
    'BTS': fillSolid('FFFFF8E1'),
    'EVENT': fillSolid('FFFBE9E7'),
    'TECH+EVENT': fillSolid('FFFFF3E0')
  };

  // ──────────────────────────────────────────────
  // SHEET 1: Content Calendar (May–Dec 2026) — V2 content verbatim, no HR column
  // ──────────────────────────────────────────────
  const s1 = wb.addWorksheet('Content Calendar', { views: [{ state: 'frozen', ySplit: 1, showGridLines: false }] });
  s1.columns = [
  ...
  h1.eachCell(c => { c.fill = fillSolid(NAVY); c.font = headerFont; c.alignment = centerAlign; c.border = border; });
  h1.height = 32;

  const posts = [
    // MAY
    { week: 1, date: 'May 4', month: 'MAY', pillar: 'TECH', postnum: 'UTCD', title: 'A Clean ILI Report Is Not Proof of Integrity', file: 'UTCD-47-OPTION-B-clean-report-not-proof.html', format: 'Carousel (7 slides)', purpose: 'Launch new UTCD 47" tool. Position through education, not announcement.', caption: 'Clean report ≠ clean pipeline. Detection limits define what you found — not what exists. UTCD 47" assembled, shipping to KSA.', gpt: 'Pipeline wall cross-section. MFL flux passing through crack undetected vs UTCD shear wave reflecting off crack. Dark bg. 1080x1350.', claude: 'IBM-style corporate. Navy/white/blue alternating. Simulated report card on slide 4. Tool spec bar slide 6.' },
    { week: 2, date: 'May 11', month: 'MAY', pillar: 'TECH', postnum: 'P13', title: "Metal Loss and Cracking Aren't the Same Threat", file: '09-metal-loss-vs-cracking.html', format: 'Carousel (6 slides)', purpose: 'Establish the MFL vs UTCD distinction. Foundation post.', caption: 'Corrosion pit = volume removed. Crack = plane separated. Different geometry, different physics, different tool.', gpt: 'Two defect types side by side. Left: rounded corrosion pit. Right: thin planar crack. Dark bg, engineering viz. 1080x1350.', claude: 'Dark theme. Side-by-side comparison slides. Blue accent cards.' },
    ...
    { week: 8, date: 'Jun 22', month: 'JUN', pillar: 'TECH', postnum: 'P03', title: "Corrosion Doesn't Grow at the Same Rate Everywhere", file: 'POST-03-corrosion-growth.html', format: 'Carousel (6 slides)', purpose: 'Growth rate education. Position re-inspection data value.', caption: 'A single inspection gives a snapshot. Successive runs give growth rates — feature by feature, not assumed globally.', gpt: 'Pipeline wall showing same corrosion feature at three time points. Progressive deepening. Growth arrows. Dark bg. 1080x1350.', claude: 'Dark theme. Three time-point comparison. Growth rate visualization.' },
  ...
  posts.forEach((p, i) => {
    ...
    row.getCell(6).font = { bold: true, size: 10, name: 'Calibri', color: { argb: NAVY } };
    if (p.file && !p.file.startsWith('—')) {
      row.getCell(7).font = { size: 10, name: 'Calibri', color: { argb: BLUE }, bold: true };
    } else {
      row.getCell(7).font = { size: 10, name: 'Calibri', color: { argb: 'FF999999' }, italic: true };
    }
    if (i % 2 === 1) {
      [1, 2, 3, 5, 8, 13].forEach(ci => { if (!row.getCell(ci).fill || !row.getCell(ci).fill.fgColor) row.getCell(ci).fill = fillSolid(ALT); });
    }
    row.height = 80;
  });

  for (let r = 2; r <= 40; r++) {
    s1.getCell(r, 13).dataValidation = { type: 'list', allowBlank: true, formulae: ['"Draft,Design,Review,Scheduled,Published"'] };
  }

  // ──────────────────────────────────────────────
  // SHEET 2: Reserve Carousels (V2 verbatim)
  // ──────────────────────────────────────────────
  const s2 = wb.addWorksheet('Reserve Carousels', { views: [{ state: 'frozen', ySplit: 1, showGridLines: false }] });
  s2.columns = [
  ...
  h2.eachCell(c => { c.fill = fillSolid(NAVY); c.font = headerFont; c.alignment = centerAlign; c.border = border; });
  h2.height = 28;
  ...
  reserve.forEach((r, i) => {
    ...
    row.eachCell(c => { c.font = bodyFont; c.alignment = wrapAlign; c.border = border; });
    row.getCell(1).alignment = centerAlign;
    row.getCell(3).font = { size: 10, name: 'Calibri', color: { argb: BLUE }, bold: true };
    if (i % 2 === 1) row.eachCell(c => { c.fill = fillSolid(ALT); });
    row.height = 45;
  });

  // ──────────────────────────────────────────────
  // SHEET 3: Holidays & Events (V2 verbatim, no HR)
  // ──────────────────────────────────────────────
  const s3 = wb.addWorksheet('Holidays & Events', { views: [{ state: 'frozen', ySplit: 1, showGridLines: false }] });
  s3.columns = [
  ...
  h3.eachCell(c => { c.fill = fillSolid(NAVY); c.font = headerFont; c.alignment = centerAlign; c.border = border; });
  ...
  const holidays = [
    ...
    ['Jun 5', 'World Environment Day', 'Sustainability', 'Global (UN)', 'No', '', ''],
    ...
    ['Sep 16', 'Intl Day for Preservation of the Ozone Layer', 'Sustainability', 'Global (UN)', 'No', '', 'Optional — emissions angle'],
    ['Sep 21', 'Zero Emissions Day', 'Sustainability', 'Global', 'No', '', 'Pipeline integrity prevents fugitive emissions'],
    ['Sep 25', 'UN SDGs Anniversary (SDG Action Day)', 'Sustainability', 'Global (UN)', 'No', '', 'SDG 9, 12, 13'],
    ['Oct 5', 'World Habitat Day', 'Sustainability', 'Global (UN)', 'No', '', 'Pipeline safety protects communities'],
    ['Oct 24', 'United Nations Day / Climate Action Week', 'Sustainability', 'Global (UN)', 'No', '', 'Broad sustainability messaging'],
    ['Dec 5', 'World Soil Day', 'Sustainability', 'Global (UN)', 'No', '', 'External corrosion / coating disbondment tie-in'],
    ['Dec 11', 'International Mountain Day', 'Sustainability', 'Global (UN)', 'No', '', 'Optional — Swiss HQ angle'],
  ];
  ...
  holidays.forEach((h) => {
    ...
    if (h[4] === 'Yes') row.getCell(5).font = { bold: true, color: { argb: 'FF2E7D32' }, size: 10, name: 'Calibri' };
    row.height = 26;
  });

  // ──────────────────────────────────────────────
  // SHEET 4: Performance Tracking (V2 verbatim)
  // ──────────────────────────────────────────────
  const s4 = wb.addWorksheet('Performance Tracking', { views: [{ state: 'frozen', ySplit: 1, xSplit: 1, showGridLines: false }] });
  const months = ['Metric', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'H2 Avg'];
  s4.addRow(months);
  s4.getRow(1).eachCell(c => { c.fill = fillSolid(NAVY); c.font = headerFont; c.alignment = centerAlign; c.border = border; });
  s4.getColumn(1).width = 26;
  for (let i = 2; i <= 10; i++) s4.getColumn(i).width = 13;
  ...
  metrics.forEach((m, i) => {
    ...
    if (i % 2 === 0) row.eachCell(c => { c.fill = fillSolid(ALT); });
    if (i < 8) {
      const rn = i + 2;
  ...
  await wb.xlsx.writeFile('C:\\Outreach\\ACME Integrity-2026-CONTENT-CALENDAR-V5.xlsx');
